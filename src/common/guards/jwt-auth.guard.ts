...
export class JwtAuthGuard extends AuthGuard(AuthStrategy.JWT) {
    ...
    async canActivate(context: ExecutionContext): Promise<any> {
        ...
        // 检查 token 是否在黑名单中
        if (await this.redis.get(genTokenBlacklistKey(token))) throw new ApiException(ErrorEnum.INVALID_LOGIN);
        ...
        try {
            ...
            if (err instanceof UnauthorizedException) throw new ApiException(ErrorEnum.INVALID_LOGIN);

            // 判断 token 是否有效且存在, 如果不存在则认证失败
            const isValid = isNil(token) ? undefined : await this.tokenService.checkAccessToken(token!);

            if (!isValid) throw new ApiException(ErrorEnum.INVALID_LOGIN);
        }
