...
export abstract class CompleteEntity extends CommonEntity {
    ...
    @VirtualColumn({ query: alias => `SELECT account FROM sys_user WHERE id = ${alias}.create_by` })
    creator: string;

    @ApiProperty({ description: "更新者" })
    @VirtualColumn({ query: alias => `SELECT account FROM sys_user WHERE id = ${alias}.update_by` })
    updater: string;
}
